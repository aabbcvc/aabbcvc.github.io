---
title: "ClickFix via WordPress for KongTuke, DDOS Botnets and Cybercriminal Feuds"
classes: wide
header:
...
Among the confirmed victims was **TGI Fridays**. Multiple other researchers have previously linked the particular IOCs we identified & the corresponding ClickFix kill-chain to be associated with **KongTuke**.
...
This decodes to `hXXps://windlrr[.]com/file.js`, which is start of the ClickFix execution chain. This domain and following execution chain has been attributed by other researchers to **KongTuke**. 
...
> This makes analysis more difficult. The ClickFix command / lures are dynamically pulled from C2, you cannot retrieve it from the source code alone. Additionally, Cloudflare has blocked the domain `nitzchi[.]com` and `windlrr[.]com` is now down. 
...
* `grabber.cy` -> This appears to be a distinct Malware-as-a-Service website. 
